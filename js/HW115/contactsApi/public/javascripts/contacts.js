(async function () {
    ...
    function updateContact(newContact) {
        ...
        theTds.find('.edit').click(async () => {
            showContactForm(newContact);
        });

        theTds.find('.delete').click(async () => {
            try{
                const response = await fetch(`/api/contacts/${newContact.id}`, {
                    method: 'DELETE'
                });
    ...
    async function loadContacts() {
        try {
            ...
            //whats the point of this 2 array functions?
            contacts
    ...
    socket.on('add', body => addContactToTable(body));
    
    socket.on('update', body => updateContact(body));

    socket.on('delete', id => removeContact(id));
})();